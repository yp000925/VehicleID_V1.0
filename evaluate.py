...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        for start_idx in count(step=args.batch_size):
            ...
            pid_matches = gallery_pids == pids[:,None]
            ...
            scores = 1 / (1 + distances)  # 此处找score最高的点
            index = np.argmax(scores, axis=1)
            index.astype(int)
            pre_pids = np.append(pre_pids, np.ndarray.tolist(np.array(gallery_pids)[index]), axis=None)
            pre_fids = np.append(pre_fids, np.ndarray.tolist(np.array(gallery_fids)[index]), axis=None)
